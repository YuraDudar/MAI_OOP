...
public:
    ...
    void deallocate(T *pointer) {
        if (pointer < &_used_blocks[0] || pointer > &_used_blocks[0] + max_count){
            throw std::logic_error("Alloc not own this ptr");
        }
    ...
    void deallocate(T *pointer, size_t n) {
        if (pointer < &_used_blocks[0] || pointer + n > &_used_blocks[0] + max_count){
            throw std::logic_error("Alloc not own this ptr");
        }
        for (int i = 0; i < n; i++){
            _free_blocks.push_back(pointer);
